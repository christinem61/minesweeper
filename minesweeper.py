...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                if (i,j) == cell:
                    continue
                elif i >= self.height or j >= self.width or i < 0 or j < 0:
                    continue
        ...
        for sentence in self.knowledge:
            ...
            if mines:
                self.knowledge.remove(sentence)
                for mine in mines.union(self.mines):
                    self.mark_mine(mine)
    ...
    def make_safe_move(self):
        ...
        """
        safeMoves = self.safes.copy() - self.moves_made
        if len(safeMoves) == 0:
            return None
        else:
    ...
    def make_random_move(self):
        ...
        while(1):
            ...
            if (i,j) in self.mines and (i,j) in self.moves_made:
                continue
            else:
                return (i,j)
        # raise NotImplementedError
